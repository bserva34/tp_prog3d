...
normalsBuffer(0), tangentsBuffer(0), uv0Buffer(0), elementsBuffer(0), bitangeantBuffer(0) {
...
void Mesh::draw() {
	...
	glBindBuffer(GL_ARRAY_BUFFER, bitangeantBuffer);
	glVertexAttribPointer(4, 4, GL_FLOAT, GL_FALSE, 3*sizeof(float), (void*)0);// 3 because assimp 
...
void Mesh::clear() {
	...
	if (bitangeantBuffer != 0) {
		glDeleteBuffers(1, &bitangeantBuffer);
	}
	if (uv0Buffer != 0) {
